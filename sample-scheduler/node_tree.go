...
func (nt *nodeTree) addNode(n *Node) {
	...
	if na, ok := nt.tree[zone]; ok {
		for _, nodeName := range na {
			if nodeName == n.name {
				fmt.Printf("Node already exists in the NodeTree: node %s, zone %s\n", n.name, zone)
				return
			}
		}
	...
	if na, ok := nt.tree[zone]; ok {
		for i, nodeName := range na {
			if nodeName == n.name {
				...
				fmt.Printf("Removed node in listed group from NodeTree: node %s, zone %s\n", n.name, zone)
				nt.numNodes--
				return nil
			}
		}
	}
	fmt.Printf("Node in listed group was not found, node %s, zone %s\n", n.name, zone)
...
func (nt *nodeTree) updateNode(old, new *Node) {
	var oldZone string
	if old != nil {
		oldZone = old.zone
	}
